...
function drawCalendar(){ // start of drawCalendar
    ...
    let currentMonthEndDays
    let weekCount = 0
    let currentMonthFirstDay = new Date(year, month, 1) // 이번달의 1일의 요일
    currentMonthFirstDay = currentMonthFirstDay.getDay() // 요일의 

    if(year%4==0){ //윤달
        currentMonthEndDays = leapYear
    }else{
        currentMonthEndDays = notLeapYear
    }

    if(currentMonthFirstDay==6&&currentMonthEndDays[month]==30 || currentMonthFirstDay==6&&currentMonthEndDays[month]==31){ // 토요일이 1일이고 그 달이 30,31일이면 6주, 
        weekCount = 6
    }else if(currentMonthFirstDay==5&&currentMonthEndDays[month]==31){ // 금요일이 1일이고 그 달이 31일이면
        weekCount = 6
    }else if(currentMonthFirstDay==0&&currentMonthEndDays==notLeapYear&&month==1){ // 윤달인데 2월이고 1일이 일요일이면 4주
        weekCount = 4
    }else{
        weekCount = 5
    }
    ...
    for(let weekOrder=0; weekOrder<weekCount; weekOrder++){
        let $tr = document.createElement('tr') 
        let prevMonthLastDay = 0
        let nextMonthFirstDay = 1
        for(let dayOrder=0; dayOrder<7; dayOrder++){
            // 요일 인덱스보다 낮으면 빈태그를 넣기
            if(weekOrder==0&&dayOrder<currentMonthFirstDay){ // 1일이 시작하는 요일 전까지는 빈태그를 넣기
                let $td = document.createElement('td')
                let $span = document.createElement('span')
                if(month==0 && dayOrder==0){ // 1월일때
                    prevMonthLastDay = currentMonthEndDays[11] - currentMonthFirstDay
                }else if(month!==0 && dayOrder==0){ // 1월아닐경우
                    prevMonthLastDay = currentMonthEndDays[month-1] - currentMonthFirstDay
                }else{
                    prevMonthLastDay = prevMonthLastDay
                }
                $span.textContent = prevMonthLastDay+1
                $span.className = "lastMonth__date"
                $td.appendChild($span)
                $tr.appendChild($td)
                prevMonthLastDay++
            }else if(date>=currentMonthEndDays[month]+1){ // 마지막날보다 크면 빈태그를 넣기
                let $td = document.createElement('td')
                let $span = document.createElement('span')
                $span.textContent = nextMonthFirstDay
